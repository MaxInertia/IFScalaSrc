...
/**
  ...
  */
case class ParPoints(P: Array[Point2D]) {

  def xSort(points: Array[Point2D]): Future[Array[Point2D]] = Future {
    points.sortWith(_.x < _.x)
  }

  def ySort(points: Array[Point2D]): Future[Array[Point2D]] = Future {
    points.sortWith(_.y < _.y)
  }

  def closestPairParallel(P: Array[Point2D]): (Point2D, Point2D, Double) = {
